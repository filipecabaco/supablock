defmodule Superblock.OAuth do
  ...
  "Build a Supabase Integration" guide:
    ...
    * `GET /v1/oauth/authorize` — authorization-code flow with **PKCE S256**
      ...
      (`http://localhost:53682/callback`, served by `Superblock.AuthCallback`);
    * `POST /v1/oauth/token` — code exchange and refresh, client id + secret
      as basic auth, form-urlencoded (per the guide);
    * `POST /v1/oauth/revoke` — used by `superblock logout` to kill the
      authorization server-side.

  These token-endpoint POSTs are the only non-GET requests superblock ever
  ...
  (`Superblock.TokenStore`) must persist the new pair before doing anything
  else.
  """
