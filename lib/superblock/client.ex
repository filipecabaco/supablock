defmodule Superblock.Client do
  @moduledoc """
  ...
  defp get_with_token(path, token, opts) do
    ...
    case Task.yield(task, budget_ms) || Task.shutdown(task, :brutal_kill) do
      ...
      {:exit, reason} ->
        Logger.debug("superblock: request #{path} crashed: #{inspect(redact(reason, token))}")
        {:error, {:transport, :crashed}}

      nil ->
        Logger.debug("superblock: request #{path} exceeded #{budget_ms}ms deadline")
        {:error, :timeout}
    end
  end
  ...
  defp run_request(path, token, budget_ms, deadline) do
    ...
    case Req.request(req) do
      ...
      {:error, %{__exception__: true} = error} ->
        Logger.debug(
          "superblock: request #{path} failed: #{redact(Exception.message(error), token)}"
        )
    ...
    kind, error ->
      Logger.debug("superblock: request #{path} raised: #{inspect(redact({kind, error}, token))}")
