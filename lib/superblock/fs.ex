defmodule Superblock.Fs do
  @moduledoc """
  FUSE-facing adapter: translates `Userfs.Fs` callbacks to `Superblock.Router`
  ...
  alias Superblock.{Control, Paths, Router}
  ...
  def userfs_getattr(state, path) do
    ...
    Logger.debug("superblock: getattr #{path} -> #{inspect(elem(result, 0))}")
  ...
  def userfs_readdir(state, path) do
    ...
    Logger.debug("superblock: readdir #{path} -> #{inspect(elem(result, 0))}")
  ...
  def userfs_read(state, path) do
    ...
    Logger.debug("superblock: read #{path} -> #{inspect(elem(result, 0))}")
  ...
  def recover_stale_mount(mountpoint) do
    if stale?(mountpoint) do
      Logger.debug("superblock: recovering stale mount at #{mountpoint}")
